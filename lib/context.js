...
class ContextSession {
  ...
  async initFromExternal() {
    ...
    const externalKey = sId.get(ctx, opts.key, opts);
  ...
  initFromCookie() {
    ...
    const cookie = sId.get(ctx, opts.key, opts);
    if (!cookie) {
      this.create();
      return;
    }
  ...
  async save(changed) {
    ...
    if (externalKey) {
      ...
      sId.set(this.ctx, key, externalKey, opts);
    ...
    sId.set(this.ctx, key, json, opts);
